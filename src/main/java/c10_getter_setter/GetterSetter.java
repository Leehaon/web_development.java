...
/*
    1. Getter / Setter의 필요성 :
        ...
        2. Setter / Getter란?
            ...
            Setter
                데이터를 "설명"하는 역할을 하는 "method", 클래스 외부에서 필드를 직접 수정하지
                못하도록 하고, 대신 메서드를 통해 값을 전달받아 수정합니다.

            Getter
                데이터를 "조화"하는 역할을 하는 "method", 클래스 외부에서 변수를 직접 읽지
                못하도록 하고, 특정 메서드를 통해 안전하게 값을 반환합니다.

                Person.java / PersonMain.java를 확인할 것 -> 응용 코드 작성

                수업 내용 확인을 위해 파일 내부르 검색하려면 -> ctrl + f
                여태까지 수업한 전체 패ㅣ지 및 파일 확인에서 내용 확인 하려면
                -> ctrl + shift + f
        ...
        4. 그래서 Setter / Getter는 언제나 필수적인가?
            모든 경우에 Setter/ Getter가 필수적인것은 "아닙니다"
            하지만 일정 사이즈 이상의 프로젝트르 진행해서 다수의 데이터를 입력 받을 경우
            적절한 데이터임을 검증하는 과정을 명시하기 위해서는
            Setter / Getter를 이용하는 것이 "효과적"입니다.

            거의 대부분 사용하기 때문에 alt + insert 등을 지원하고
            추후 수업할 lombok 애너데이션을 사용하는 편입니다.
            이는 객체 지향 프로그래밍의 중효한 기초이며, 안전한 클린코드를 작성하는 과정이라는
            측면에서 매우 중요합니다.
 */
