...
public class Factory {
    ...
    // 생성자 정의 부분 필요 없음 -> 에너테이션 때문에

    // 메서드 정의 -> 이제 중요한 점은 return타입이 String, int 이런게 아니라
    // 객체가 수업 중에 정의한 클래스라는 점입니다. -> 싱글톤에서도 했습니다. Samsung.java 확인
    public SmartPhone produceSmartPhone() {
        ...
        Samsung samsung = Samsung.getInstance();    // -> produceSmartPhone()을 처음 실행할 때서야
                                                    ...
                                                    // 이 후 두 번째 produceSmartPhone()을 실행할 때는
                                                    // 이미 만들어진 instance가 반환됨.
        serial = samsung.scoreSerialNumber(model);

        SmartPhone smartPhone = new SmartPhone(samsung.getCompany(), model, serial);
        // samsung.로 치게 될 경우 자동완성으로 getCompany()가 있습니다.
        // 그 이유는 Samsung.java 확인했을 때 company 필드에만 @Getter가 적용돼있고
        // serialNumber에는 @Getter가 없었기 때문입니다. 즉, 매번 클래스 단위에 일단 @Data 에너테이션을
        // 적용하는 방식은 비효율적이고 코드 가독성을 떨어트릴 수 있어 명확한 설계 하여 코딩을 할 필요가 있습니다.
        return smartPhone;
