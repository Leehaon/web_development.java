...
/*
     travelplan 패키지 생성
     TravelPlan.java
     TravelPlanMain.java 파일 생성

     응용 과제 :
        여행 계획 객체 생성

    다음과 같은 속성을 가지 여행 계획(travelPlan) 객체를 생성해야 함.
        1. 여행지(destination) : 파리
        2. 출발일(depatureDate) : 2024 - 12 - 17
        3. 귀국일(returnDate) : 2024 - 12 - 26
        4. 여행자 수(numberOfTravelers) : 2
        5. 호텔 이름(hotelName) : 아난타코브
        6. 비행기번호(flightNumber) : AF123
...
public class TravelPlanMain {
    public static void main(String[] args) {
        ...
        TravelPlan travelPlan1 = TravelPlan.builder()       // 여기서는 new가 안들어갔었습니다.
                .destination("파리")                         // 이유는 Builder에서 필수 생성자가 없기 때문에
        ...
        TravelPlanNoLombok travelPlanNoLombok1 = new TravelPlanNoLombok.Builder("런던", "2025 - 1 - 1")
            .build();   // 하지만 TravelPlanNoLombok과 Builder에서 final로 지정한 속성 때문에
                        // Builder의 생성자를 이용하는 경우 new와 .Builder(인수들)로 대문자 Builder를 사용했기 때문이다.
