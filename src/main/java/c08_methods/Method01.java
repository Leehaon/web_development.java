...
/*
    ...
    Method : 일반의 동작을 묶어서 정의한 것. 입력값과 출력값이 존재(optional)

    함수와 매서드가 동일한 개념이라면 왜 용어를 분리하는가?
        -> 클래스 내부에 함수를 정의하게 되면 이를 매서드라고 칭함.
        -> 자바의 경우 모든 함수들이 특정 클래스에 종속돼있기 때문에 전부 다
           메서드에 해당함
    ...
    미리 입력(메서드 정의)해놓고 나서, '사진을 찍는다'라는 명령어를 실행(메서트 호출) 시키면
    자동으로 1) ~ 5)까지의 명령을 순서대로 수행함
    -> 즉 메서드란 순차적으로 이루어지는 명령어들을 모아둔 것

    2. 메서드 관련 용어 정리
        1) 메서드 정의(define) : 사용자가 메서드르르 새로 만드는 것을 의미
 ...
 */
    ...
    // 3번 유형 -> [ x | o ] : void가 아니라 출력값(return)의 자료형 명시해줌.
    ...
    // 4번 유형 -> [ o | o ] : call2()가 하나의 매개변수만 가졌던 것과 달리 call4()는 다수의 매개변수를 가질 수 있다.
    public static String call4(int year, int month, int date) {
        /**
         *  DoxString : 클래스나 메서드에 대한 설명을 하기 위한 주석 처리 방법
...
//        routine = scanner.nextLine();   // -> "대회 신청"이라고 입력했다면, 그것은 scanner.nextLine()의 return값
...
//        System.out.println(call4(2024, 11, 29));  // 실행 방법 1 : 출령문 내에 바로 함수 호출
        // 변수 선언 및 초기화  -> 메서드의 return 값을 todayDate에 대입
//        String todayDate = call4(2024, 11, 29);
//        System.out.println(todayDate);      // 실행 방법 2 : 별도의 변수르 선언하여 메서드의 return값을 저장 후, 변수를 통해 sout 실행
        ...
        String myBithday = call4(year, month, day);
