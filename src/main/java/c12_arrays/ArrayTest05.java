...
/*
    디자인 배열(Multi Dimensional Array) 중에 2차원 배열

    다차원 배열은 2차원 배열을 의미하지만, 개발 환경 상 2차원 배열 이상을 사용하는 경우는 극히
    드믈기 때문에 2차원 배열을 기준으로 강의합니다.
...
//                for (int j = 0; j< arr3.length; j++) {
                    // 이사의 코드에서 arr3[i]가 1차 배열임을 확인할 수 있습니다 -> 배열에. length 필드가 있으니까
...
//                System.out.println("Arrays.toString() : " + Arrays.toString(arr3));
                ...
                // 4차원 배열의 출력 방식 Arrays.deepToString(배열이름)
//                System.out.println("Arrays.deepToString() : " + Arrays.deepToString(arr3));
//            }

        // 2차원 배열 prettyWriting으로 작성
        int[][] arr = {
                { 1, 2, 3 },
                { 4, 5, 6 },
                { 7, 8, 9 }
